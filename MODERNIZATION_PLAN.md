...
| World mutation inside a v19 collision callback (post-Phase 4) | `BirdEntity.onCollision` removed the score `hit` with `removeChildNow`, which synchronously nulls the hit's `body` **mid collision-loop**; v19 then read `other.body.isStatic` → `Cannot read properties of undefined (reading 'isStatic')`, crashing on reaching the first pipe. The e2e "score increments" spec missed it because it called `onCollision` **directly**, bypassing the engine loop. | ✅ **Closed** — switched to the **deferred `removeChild`** + one-shot `scored` guard + `hit` body `setSensor(true)`; the scoring e2e spec now drives the **real collision engine** and asserts **0 errors** (reproduces the crash on the pre-fix code, green after) |
| Bird hitbox offset from its sprite (post-Phase 4) | melonJS v19's `me.Ellipse(x,y,w,h)` takes x,y as the ellipse **center**, but the v4 original treated them as the **top-left**. The bird's shape was ported verbatim as `new me.Ellipse(5,5,71,51)`, centering the hitbox at (5,5) — ~30px left / ~20px above the sprite, half off the bird. The visible bird flew clean through a gap while its offset hitbox clipped the pipe/ceiling: **"die no matter what going through the pipe"**. The pipe-only "unwinnable-wall" spec didn't check the bird. | ✅ **Closed** — pass the intended **center** `(5+71/2, 5+51/2)`; a new e2e regression asserts the bird's collision body sits inside its render bounds and is centered on the sprite (fails on the mis-centered ellipse, green after). Verified with a body-vs-render overlay screenshot |
